...
// TODO: Replace with real API data when the analytics endpoint is ready.

/** Monthly subscription growth figures for the bar chart */
const barData = [
  { name: "Jan", value: 45 },
  { name: "Feb", value: 52 },
  { name: "Mar", value: 65 },
...
/** Revenue share per top sales person for the donut chart */
const pieData = [
 ...
 * SuperAdminDashboard
 * ─────────────────────────────────────────────────────────────
 * Overview dashboard for the Super Admin role.
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-slate-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <StatCard
          ...
          value={dashboardData?.summary?.totalSalesman || 0}
          trend="+12%"
          icon={<Users size={24} />}
          color="bg-purple-600"
        />
        ...
        <StatCard
          ...
          value={dashboardData?.summary?.totalSubscriptionsSold || 0}
          trend="+18%"
      ...
      {/* ── 2. Main Grid (left content + right widgets) ── */}
      <div className="grid grid-cols-12 gap-8">

        {/* ── Left Column: Table + Charts ── */}
        <div className="col-span-12 lg:col-span-8 space-y-8">

          {/* ── Sales Executive Management Table ── */}
...
/**
 * StatCard
 * Displays a single summary metric with a trend indicator and colored icon.
 *
 * @param {string}    title  - Label shown above the value
 * @param {number}    value  - The numeric stat
 * @param {string}    trend  - Trend text e.g. "+12%"
 * @param {ReactNode} icon   - Lucide icon element
 * @param {string}    color  - Tailwind bg color class for the icon container
 */
...
/**
 * TableRow
 * A single row in the Sales Executive Management table.
 */
...
/**
 * AlertItem
 * A card shown inside the Low Performance widget for underperforming executives.
 */
const AlertItem = ({ name, sub }) => (
  <div className="bg-white p-3 rounded-xl flex justify-between items-center shadow-sm">
    ...
    <button className="border border-red-200 text-red-600 px-3 py-1 rounded-lg text-xs font-bold hover:bg-red-50">
      Notify
    </button>
  </div>
);
